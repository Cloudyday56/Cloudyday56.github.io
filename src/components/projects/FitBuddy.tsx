...
const FitBuddy = () => {
  return (
    <div className="w-full flex flex-col items-center gap-2">
      ...
      {/* Middle row: Image and Features (md+ only) */}
      <div className="w-full flex flex-col md:flex-row items-center justify-center gap-4">
        ...
        <ul className="hidden md:block list-disc list-outside space-y-2 pl-6 text-sm 2xl:text-lg text-gray-400 bg-base-200 rounded-lg p-4 w-full md:w-2/5 max-w-xs 2xl:max-w-md">
          {features.map((f, i) => (
      ...
      <div className="w-full max-w-3xl 2xl:max-w-5xl px-8">
        ...
        {/* Features list for mobile only */}
        <ul className="block md:hidden list-disc list-outside space-y-2 pl-6 text-sm 2xl:text-lg text-gray-400 bg-base-200 rounded-lg p-4 w-full max-w-xs 2xl:max-w-md mt-4">
          {features.map((f, i) => (
            <li key={i}>{highlightFeatureText(f)}</li>
          ))}
        </ul>
